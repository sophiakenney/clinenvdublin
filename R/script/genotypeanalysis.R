...
# Create matrix for clustering strains according to similarity in plot
argmat <- arg %>%
  mutate(value = 1) %>%
...
arg2 <- reshape2::melt(argmat2, id.vars = "ID")

arg2 <- merge(arg2 %>% mutate(Gene.symbol = variable) %>% select(-c(variable)),
              arg %>% filter(!str_detect(ID, "ARS16")) %>% 
                ...
                mutate(Element.subtype = ifelse(Element.subtype == "Amr", "AMR", Element.subtype)), by=c("ID", "Gene.symbol"))
...
# Efflux genes can be found in amrfinder/prokka_anno.txt
efflux <- rbind(arg %>%
                  filter(!str_detect(ID, "ARS16")) %>% 
